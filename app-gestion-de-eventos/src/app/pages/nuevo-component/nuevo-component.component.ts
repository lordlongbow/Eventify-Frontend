...
nuevo() {
  if (this.nuevoForm.valid) {
    ...
    if (fotoInput instanceof File) {
      ...
      console.log('Archivo foto agregado al FormData:', fotoInput);
    } else {
    ...
    console.error('Formulario inválido:', this.nuevoForm.value);
  ...
  onFileSelected(event: any): void {
    ...
    reader.onload = () => {
      ...
      console.log('Vista previa generada con FileReader:', this.previewImage);
    };
    reader.readAsDataURL(file);
  
    // Asignar el archivo al control del formulario
    this.nuevoForm.patchValue({ foto: file });
    this.nuevoForm.get('foto')?.updateValueAndValidity(); // Validar el campo después de actualizarlo
    console.log('Archivo almacenado en el formulario:', this.nuevoForm.get('foto')?.value);
  }
